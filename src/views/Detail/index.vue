    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <!-- 因为数据获取是异步的，当传数据过去可能遇到对象无数据，导致undefined的属性报错 -->
          <Zoom
          ...
          <ImageList
            :skuImageList="skuInfo.skuImageList || []"
            @changeIndex="defaultIndex = $event"
          />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="Num" readonly />
...
<script>
  ...
  export default {
    ...
    methods: {
      async getDetailList() {
        const result = await reqDetailList(this.$route.params.skuId);
      ...
      // 排他选中属性
      changeIschecked(itemIschecked, index) {
        itemIschecked.forEach((item) => {
          item.isChecked = "0";
        });
        itemIschecked[index].isChecked = "1";
      },
      ...
      async addOrUpdateCart() {
        await reqAddOrUpdateCart(this.skuInfo.id, this.Num);
        alert("购物车添加成功");
        // 将在购物车成功的页面需要展示的数据放入sessionStorage
